...
const double syncmisalign = 0.4;    // maximum time mis-alignment before re-sync
...
void uiEvent(mjuiState* state)
{
    ...
    if( state->type==mjEVENT_KEY && state->key!=0 )
    {
        ...
        case mjKEY_RIGHT:           // step forward
            ...
            // if (state->alt){
            //     if (CTL_PARAM::comd_pm.lipm_ys < CTL_PARAM::comd_pm.lipm_ys_minax(1)){
            //         CTL_PARAM::comd_pm.lipm_ys += 0.04;
            //     }
            // }else if(state->control){
            //     if (CTL_PARAM::comd_pm.move_dir > CTL_PARAM::comd_pm.lipm_dir_minax(0)){
            //         CTL_PARAM::comd_pm.move_dir -= 0.04;
            //     }
            // }
            // else{
            //     if (CTL_PARAM::comd_pm.move_side > CTL_PARAM::comd_pm.lipm_side_minax(0)){
            //         CTL_PARAM::comd_pm.move_side -= 0.04;
            //     }
            // }
        ...
        case mjKEY_LEFT:            // step back
            ...
            // }
            // if (state->alt){
            //     if (CTL_PARAM::comd_pm.lipm_ys > CTL_PARAM::comd_pm.lipm_ys_minax(0)){
            //         CTL_PARAM::comd_pm.lipm_ys -= 0.04;
            //     }
            // }else if(state->control){
            //     if (CTL_PARAM::comd_pm.move_dir < CTL_PARAM::comd_pm.lipm_dir_minax(1)){
            //         CTL_PARAM::comd_pm.move_dir += 0.04;
            //     }
            // }
            // else{
            //     if (CTL_PARAM::comd_pm.move_side < CTL_PARAM::comd_pm.lipm_side_minax(1)){
            //         CTL_PARAM::comd_pm.move_side += 0.04;
            //     }
            // }
        ...
        case mjKEY_DOWN:            // step forward 100
            ...
            // if (CTL_PARAM::comd_pm.lipm_xs > CTL_PARAM::comd_pm.lipm_xs_minax(0)){
            //     CTL_PARAM::comd_pm.lipm_xs -= 0.04;
            // }
        ...
        case mjKEY_UP:              // step back 100
            ...
            // if (CTL_PARAM::comd_pm.lipm_xs < CTL_PARAM::comd_pm.lipm_xs_minax(1)){
            //     CTL_PARAM::comd_pm.lipm_xs += 0.04;
            // }
        ...
        case mjKEY_TAB:
            // if(state->control){
            //     if(CTL_PARAM::leg_pm.ankle_mode == PASSIVE){
            //         CTL_PARAM::leg_pm.ankle_mode = ACTIVE;
            //     }else{
            //         CTL_PARAM::leg_pm.ankle_mode = PASSIVE;
            //     }
            // }
            break;
        ...
        case mjKEY_HOME:
            ...
            // if ( video_rec.record_status == VideoRec::hold ){
            //     mj_init_recording(&video_rec, "./rec.mp4", 1920, 1080, window);
            //     video_rec.end_t = get_microseconds();
            //     SHOW_INFO("mj_init_recording");
            // }
            // if( video_rec.record_status == VideoRec::rec ){
            //     mj_close_recording(&video_rec);
            //     SHOW_INFO("mj_close_recording");
            //     video_rec.record_status = VideoRec::hold;
            // }
            break;
    ...
    if( state->type==mjEVENT_MOVE && state->dragrect==3 && m )
    {
        ...
        if( pert.active )
            ...
            mjv_moveCamera(m, action, 2*state->dx/r.height, -2*state->dy/r.height,
                           &scn, &cam);
...
void render(GLFWwindow* window)
{
    ...
    mj_infotext(mjb_comd_title, mjb_comd_info);
    mjr_overlay(mjFONT_NORMAL, mjGRID_TOPLEFT, rect,
                    mjb_comd_title, mjb_comd_info, &con);
        ...
        if( m )
        {
            ...
            if( settings.run )
            {
                    ...
                    mju_abs((d->time-simsync)-(tmstart-cpusync))>syncmisalign )
                {
                    ...
                    // SHOW_VAL4("仿真与真实时间未对齐", d->time-simsync, tmstart-cpusync, cpusync, mju_abs((d->time-simsync)-(tmstart-cpusync)));
                    // re-sync
                    cpusync = tmstart;
                    simsync = d->time;
                ...
                else
                {
                           ...
                           (glfwGetTime()-tmstart)<refreshfactor/vmode.refreshRate )
                    {
                        ...
                        // SHOW_VECTOR("vmode.refreshRate", vmode.refreshRate);
                        // SHOW_VECTOR("glfwGetTime()", glfwGetTime());
                        // SHOW_VECTOR("d->time", d->time);

                        // **********
                        // 主要逻辑部分
                        mj_main_loop(m, d, peroid_record);
                        peroid_record = peroid_control(1, 1000 * TIME_STEP);
                        // **********
                        // run mj_step
...
int main(int argc, const char** argv)
{
    ...
    while( !glfwWindowShouldClose(window) && !settings.exitrequest )
    {
        ...
        if (video_rec.record_status == VideoRec::rec || video_rec.record_status == VideoRec::init){
            video_rec.sleep_time = get_microseconds() - video_rec.end_t;
            if(video_rec.sleep_time >= video_rec.sleep_peroid){
                mj_record_frame(&video_rec, window, &con);
                video_rec.end_t = get_microseconds();
            }
        }
